...
export class AnthropicProxy {
  ...
  private async handleProxyRequest(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const isValidUser = await this.validatePostgresCredentials(username, password);
      if (!isValidUser) {
  ...
  private async validatePostgresCredentials(
    ...
    try {
      ...
      logger.debug(`PostgreSQL auth failed for user ${username}:`, (error as Error).message);
      return false;
    } finally {
      try {
        await client.end();
  ...
  private async forwardToAnthropic(req: Request, res: Response): Promise<void> {
    ...
    if (isOAuthToken && req.path === '/v1/messages' && !anthropicUrl.includes('beta=')) {
    ...
    logger.info(`🔧 Original body type: ${typeof body}, length: ${body ? (typeof body === 'string' ? body.length : JSON.stringify(body).length) : 0}`);
    ...
    if (isOAuthToken) {
      ...
      try {
        if (body) {
          ...
          logger.info(`🔧 Parsed request data:`, { model: requestData.model, hasMessages: !!requestData.messages });
        }
      } catch (e) {
      ...
      const isExpensiveModel = ['sonnet', 'opus'].some(tier => model.toLowerCase().includes(tier));
      
      if (isExpensiveModel) {
        logger.info(`🎯 Expensive model detected (${model}) - enhancing request for Claude Code CLI compatibility`);
        ...
        if (enhancedBody.messages && enhancedBody.messages[0]) {
        ...
        logger.info(`📝 Enhanced body for expensive model (length: ${body.length})`);
      } else {
      ...
      if (isExpensiveModel) {
        ...
        headers["anthropic-beta"] = "oauth-2025-04-20,fine-grained-tool-streaming-2025-05-14";
      }
      
      logger.info(`🔧 Using OAuth token with Claude Code CLI pattern for ${isExpensiveModel ? 'expensive' : 'cheap'} model`);
    } else {
    ...
    try {
      ...
      res.status(502).json({ error: "Bad gateway - failed to reach Anthropic API" });
    }
  }
}
 ...
 */
export function createAnthropicProxy(config: AnthropicProxyConfig): AnthropicProxy {
