...
export function buildGatewayConfig(): GatewayConfig {
  ...
  logger.debug("Config debug:", {
    ...
    slackAppToken: config.slack.appToken
      ? `${config.slack.appToken.substring(0, 10)}...`
      : undefined,
    socketMode: config.slack.socketMode,
    mcpServersUrl: config.mcp.serversUrl,
    publicGatewayUrl: config.mcp.publicGatewayUrl,
  });
 ...
 */
export function displayConfig(config: GatewayConfig): void {
  ...
  console.log(
    `  Bot Token: ${config.slack.token?.substring(0, DISPLAY.TOKEN_PREVIEW_LENGTH)}... (${config.slack.token.length} chars)`
  );
  console.log(
    `  App Token: ${config.slack.appToken ? `${config.slack.appToken.substring(0, DISPLAY.TOKEN_PREVIEW_LENGTH)}... (${config.slack.appToken.length} chars)` : "not set"}`
  );
  console.log(`  API URL: ${config.slack.apiUrl}`);
