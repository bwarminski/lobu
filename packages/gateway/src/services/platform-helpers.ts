...
/**
 * Simple message deduplication with automatic trimming.
 * Prevents processing the same message ID twice.
 */
export class MessageDeduplicator<T extends string | number = string> {
  ...
  /**
   * Mark an id as seen without checking.
   */
  markSeen(id: T): void {
    this.seen.add(id);
    this.trim();
